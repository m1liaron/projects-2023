window.addEventListener('load',function(){
    ...
    class Player {
        constructor(game){
            ...
            // Центральна точка гравця зіткнення 
            ...
            this.spriteX;
            this.spriteY;
            // Верхні лівий кут кадру аркуша спрайтів 
            this.frameX = 0;
            this.frameY = 0;
            this.image = document.getElementById('bull');
            // Ми передаємо таке щоб гравець також не виходив за межи
            
        }
        draw(context){
            context.drawImage(this.image, this.frameX * this.
            spriteWidth, this.frameY * this.spriteHeight, 
            this.spriteWidth, this.spriteHeight, this.spriteX, 
            this.spriteY, this.width, this.height);
            if(this.game.debug){
                context.beginPath();
            context.arc(this.collisionX,this.collisionY,50,
                0,Math.PI * 2);
        ...
        update(){
            ...
            this.game.obstacles.forEach(obstacle => {
                ...
                // let collision1 = game.checkCollision(this,
                //     obstacle)[0]
                // let distance1 = game.checkCollision(this,
                //     obstacle)[1]
                if(collision){
                    const unit_x = dx / distance;
                    const unit_y = dy / distance;
                    this.collisionX = obstacle.collisionX + (sumOfRadii + 1) *  unit_x;
                    // Беремо експозицію центральної точки гравців.Коло зіткнення щоб висунити його за межи радіуса перешкоди, з якою він стискається. Ми рухаємо його горизонтально до положення центру точка кола перешкод плюс сума радіуси кола гравця та перешкоди коло плюс один  додатковий піксель поза ним помножити на одиницю співвідношення Х,щоб дати йому правильному напрямку від перешкоди центрольної точки.
                    this.collisionY = obstacle.collisionY + (sumOfRadii + 1) * unit_y
                    // Цей код висуває гравця на один піксель за межиі радіус зіткнення з перешкодою в напрямку від центрольної точки 
                }
            })
        }
    }
    ...
    class Obstacle {
        ...
        draw(context){
            context.drawImage(this.image, this.frameX * this.spriteWidth ,
                this.frameY * this.spriteHeight, this.spriteWidth, this.spriteHeight, this.
                spriteX, this.
                spriteY, this.width, this.height);
            // 3 аргумента перше це зобреження 
            if(this.game.debug){
                context.beginPath();
    ...
    class Enemy {
        constructor(game){
            ...
            this.collisionY = this.game.topMargin + (Math.random() * (this.game.height - this.game.
            topMargin));
            this.spriteX;
            this.spriteY;
        }
    ...
    class Game {
        constructor(canvas){
            ...
            // Щоб наша гру була теж самої висоти що ї малюнок^_^
            this.topMargin = 260;
            this.debug = true;   
            this.player  = new Player(this);
            this.fps = 70;
            this.timer = 0;
            // рахує знову і знову від 0 до а конкретне значення,коли воно досягне цього значення, яке запустить наступну анімацію кадру, і його буде скинуто до 0 
            this.interval = 1000/this.fps;
            // Значення контрольної точки що при досягненні скине таймер
            this.eggTimer = 0;
        ...
        render(context,deltaTime){ // deltaTime для відтворення методу
        ...
        checkCollision(a, b){
            ...
            // Різниця між центром кола А і центральна точка кола В на вертикальна вісь у
